...
function loadEnv(): void {
  ...
  try {
    // Try .env file first - it should override platform env vars
    try {
      ...
      if (result.parsed && result.parsed.COZE_SUPABASE_URL && result.parsed.COZE_SUPABASE_ANON_KEY) {
        ...
        if (result.parsed.COZE_SUPABASE_SERVICE_ROLE_KEY) {
          process.env.COZE_SUPABASE_SERVICE_ROLE_KEY = result.parsed.COZE_SUPABASE_SERVICE_ROLE_KEY;
        }
        envLoaded = true;
        return;
      }
